                            ...
                            const ErrorCallback& ecb)
{
    ...
    if (m_isMalicious && !dst.IsBroadcast() && !IsMyOwnAddress(dst))
    {
        static const double kBlackholeDropProbability = 0.5;
        if (m_uniformRandomVariable == nullptr)
        {
            m_uniformRandomVariable = CreateObject<UniformRandomVariable>();
        }
        const double draw = m_uniformRandomVariable->GetValue(0.0, 1.0);
        if (draw < kBlackholeDropProbability)
        {
            NS_LOG_INFO("BLACKHOLE: Dropping packet toward " << dst);
            return true;
        }
        NS_LOG_INFO("BLACKHOLE: Rarely forwarding packet toward " << dst);
    }
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    // Simple blackhole behavior: reply dengan RREP palsu lalu hentikan proses normal
    if (m_isMalicious)
    {
        ...
        RrepHeader rrepHeader(/*prefixSize=*/0,
                              ...
                              /*dstSeqNo=*/rreqHeader.GetDstSeqno() + 4,
        ...
        Ptr<Socket> socket =
            FindSocketWithInterfaceAddress(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0));
        if (socket)
        {
