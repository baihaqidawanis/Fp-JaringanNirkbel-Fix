 ...
 */
#include "ns3/aodv-trust-neighbor.h"
#include "ns3/aodv-trust-packet.h"
#include "ns3/aodv-trust-rqueue.h"
#include "ns3/aodv-trust-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("AODVTRUST TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(AODVTRUSTTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), AODVTRUSTTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(AODVTRUSTTYPE_RREP);
 ...
 * \ingroup aodv-trust-test
 *
 * \brief Unit test for AODVTRUST routing table entry
 */
...
struct AodvTrustRqueueTest : public TestCase
{
    AodvTrustRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
AodvTrustRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&AodvTrustRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&AodvTrustRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &AodvTrustRqueueTest::CheckTimeout, this);
...
void
AodvTrustRqueueTest::CheckSizeLimit()
{
...
void
AodvTrustRqueueTest::CheckTimeout()
{
 ...
 * \brief Unit test for AODVTRUST routing table entry
 */
struct AodvTrustRtableEntryTest : public TestCase
{
    AodvTrustRtableEntryTest()
 ...
 * \ingroup aodv-trust-test
 *
 * \brief Unit test for AODVTRUST routing table
 */
struct AodvTrustRtableTest : public TestCase
{
    AodvTrustRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup aodv-trust-test
 *
 * \brief AODVTRUST test suite
 */
class AodvTrustTestSuite : public TestSuite
{
  public:
    AodvTrustTestSuite()
        : TestSuite("routing-aodvtrust", UNIT)
    {
        ...
        AddTestCase(new AodvTrustRqueueTest, TestCase::QUICK);
        AddTestCase(new AodvTrustRtableEntryTest, TestCase::QUICK);
        AddTestCase(new AodvTrustRtableTest, TestCase::QUICK);
    }
} g_aodvtrustTestSuite; ///< the test suite
