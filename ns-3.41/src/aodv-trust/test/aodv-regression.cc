...
#include "aodv-trust-regression.h"
 ...
 *
 * \brief AODVTRUST regression test suite
 */
class AodvTrustRegressionTestSuite : public TestSuite
{
  public:
    AodvTrustRegressionTestSuite()
        : TestSuite("routing-aodv-trust-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("aodv-trust-chain-regression-test"), TestCase::QUICK);
...
} g_aodvtrustRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("AODVTRUST chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & AODVTRUST
    AodvTrustHelper aodvtrust; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(aodvtrust);
    ...
    streamsUsed += aodvtrust.AssignStreams(*m_nodes, streamsUsed);
    // AODVTRUST uses m_size more streams
