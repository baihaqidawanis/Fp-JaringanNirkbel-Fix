 ...
 */
#ifndef AODVTRUSTROUTINGPROTOCOL_H
#define AODVTRUSTROUTINGPROTOCOL_H
 ...
 * \ingroup aodvtrust
 *
 * \brief AODVTRUST routing protocol
 */
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    static const uint32_t AODVTRUST_PORT;
    ...
    // --- FITUR BLACKHOLE ATTACK (SEKARANG PUBLIC AGAR BISA DIAKSES MAIN) ---
    void SetMalicious (bool f); // Fungsi untuk mengaktifkan mode jahat
    bool GetMalicious () const;
    void BlacklistAddress (Ipv4Address neighbor);
    // -----------------------------------------------------------------------

  protected:
    void DoInitialize() override;

    // --- MODIFIKASI AODV-TRUST START ---
    /// Menyimpan nilai Trust untuk setiap tetangga (IP -> Score 0.0 s/d 1.0)
    std::map<Ipv4Address, double> m_trustTable;

    /// Fungsi untuk update nilai trust
    void UpdateTrust (Ipv4Address neighbor, double value);

    /// Fungsi untuk mengecek apakah node aman (Trust > threshold)
    bool IsTrusted (Ipv4Address neighbor);
    
    /// Konstanta Threshold (Batas aman)
    double m_trustThreshold;
    // --- MODIFIKASI AODV-TRUST END ---

    // --- VARIABLE BLACKHOLE (TETAP PROTECTED) ---
    bool m_isMalicious; // Penanda apakah node ini jahat
    // --------------------------------------------
  ...
  private:
     ...
     */
    void RecvAodvTrust(Ptr<Socket> socket);
    /**
     * Receive RREQ
     * \param p packet
...
#endif /* AODVTRUSTROUTINGPROTOCOL_H */