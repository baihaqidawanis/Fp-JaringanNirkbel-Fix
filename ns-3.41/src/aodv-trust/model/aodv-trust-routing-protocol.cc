 ...
 *
 * Based on
 * NS-2 AODVTRUST model developed by the CMU/MONARCH group and optimized and
 * tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 * aodv-trust-UU implementation by Erik Nordström of Uppsala University
 * https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/aodv-trust-UU
 *
...
/// UDP Port for AODVTRUST control traffic
const uint32_t RoutingProtocol::AODVTRUST_PORT = 654;

/**
 * \ingroup aodvtrust
 * \brief Tag used by AODVTRUST implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::aodvtrust::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("AodvTrust")
...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      // --- MODIFIKASI TRUST START ---
    m_trustThreshold(0.15), // Inisialisasi Threshold Trust
      // --- MODIFIKASI TRUST END ---
      // --- FITUR BLACKHOLE START ---
      m_isMalicious(false)   // Default tidak jahat
      // --- FITUR BLACKHOLE END ---
{
...
{
    static TypeId tid =
        TypeId("ns3::aodvtrust::RoutingProtocol")
            ...
            .AddAttribute ("IsMalicious",
                           "Set true to simulate a Blackhole Malicious node.",
                           BooleanValue (false),
...
bool
RoutingProtocol::GetMalicious () const
{
...
RoutingProtocol::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    *stream->GetStream() << "Node: " << m_ipv4->GetObject<Node>()->GetId()
                         ...
                         << ", AODVTRUST Routing table" << std::endl;
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No aodvtrust interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
    ...
    if (m_routingTable.LookupValidRoute(dst, rt))
    {
        ...
        NS_LOG_DEBUG("Exist route to " << route->GetDestination() << " from interface "
                                       << route->GetSource());
        if (!IsTrusted(rt.GetNextHop()))
        {
            NS_LOG_LOGIC("TRUST SYSTEM: Menghindari next hop tidak dipercaya " << rt.GetNextHop());
            rt.SetFlag(INVALID);
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // --- FITUR BLACKHOLE ATTACK START ---
    // Jika saya node jahat, dan paket ini BUKAN untuk saya (artinya saya cuma perantara)
    if (m_isMalicious)
    {
        if (!IsMyOwnAddress(header.GetDestination())) 
        {
            NS_LOG_INFO ("BLACKHOLE: Dropping Data Packet " << p->GetUid() << " from " << header.GetSource() << " to " << header.GetDestination());
            return true; // Return true seolah-olah sukses diproses, padahal dibuang (DROP)
        }
    }
    // --- FITUR BLACKHOLE ATTACK END ---

    // AODVTRUST is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == AODVTRUST_PORT)
                    {
                        // AODVTRUST packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
                            ...
                            ErrorCallback ecb)
{
    ...
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        {
            if (!IsTrusted(toDst.GetNextHop()))
            {
                NS_LOG_LOGIC("TRUST SYSTEM: Menolak forward melalui next hop tidak dipercaya "
                             << toDst.GetNextHop());
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("AODVTRUST does not work with more then one address per each interface.");
    }
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodvTrust, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), AODVTRUST_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodvTrust, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), AODVTRUST_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("AODVTRUST does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in AODVTRUST operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when AODVTRUST does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // AODVTRUST needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available AODVTRUST interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid AODVTRUST source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by aodvtrust
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(AODVTRUSTTYPE_RREQ);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, AODVTRUST_PORT));
}
...
void
RoutingProtocol::RecvAodvTrust(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("AODVTRUST node " << this << " received a AODVTRUST packet from " << sender << " to "
                                   << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("AODVTRUST message " << packet->GetUid() << " with unknown type received: "
                                          << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    {
    case AODVTRUSTTYPE_RREQ: {
        RecvRequest(packet, receiver, sender);
        break;
    }
    case AODVTRUSTTYPE_RREP: {
        RecvReply(packet, receiver, sender);
        break;
    }
    case AODVTRUSTTYPE_RERR: {
        RecvError(packet, sender);
        break;
    }
    case AODVTRUSTTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    }
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    // --- FITUR BLACKHOLE ATTACK START ---
    // Jika saya node jahat, saya akan berbohong bahwa saya punya rute ke tujuan
    if (m_isMalicious)
    {
        NS_LOG_INFO ("BLACKHOLE: Intercepting RREQ from " << src << ". Sending Fake RREP.");
        
        // Buat RREP Palsu
        // Kita set SeqNo sangat tinggi agar dianggap rute 'terbaru' dan 'terbaik'
        RrepHeader rrepHeader (/*prefixSize=*/0,
                               /*hopCount=*/1, // Ngaku dekat (1 hop)
                               /*dst=*/rreqHeader.GetDst(),
                               /*dstSeqNo=*/rreqHeader.GetDstSeqno() + 4, // FAKE SEQ NO TINGGI
                               /*origin=*/rreqHeader.GetOrigin(),
                               /*lifetime=*/m_myRouteTimeout);

        Ptr<Packet> packet = Create<Packet> ();
        SocketIpTtlTag tag;
        tag.SetTtl (rreqHeader.GetHopCount () + 1);
        packet->AddPacketTag (tag);
        packet->AddHeader (rrepHeader);
        TypeHeader tHeader (AODVTRUSTTYPE_RREP);
        packet->AddHeader (tHeader);
        
        // Kirim balasan palsu ke pengirim
        Ptr<Socket> socket = FindSocketWithInterfaceAddress (m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0));
        socket->SendTo (packet, 0, InetSocketAddress (src, AODVTRUST_PORT));
    }
    // --- FITUR BLACKHOLE ATTACK END ---
    ...
    if (!IsTrusted (src))
    {
        NS_LOG_LOGIC ("TRUST SYSTEM: Dropping RREQ from Untrusted Node: " << src);
        return;
    }

    UpdateTrust (src, 0.02);
    // --- MODIFIKASI TRUST END ---
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), AODVTRUST_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(AODVTRUSTTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), AODVTRUST_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(AODVTRUSTTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, AODVTRUST_PORT));
}
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    Ipv4Address dst = rrepHeader.GetDst(); // Destination (Tujuan Akhir)

    // --- MODIFIKASI TRUST SYSTEM: DETEKSI ANOMALI ---
    // Di simulasi Blackhole, penyerang menaikkan SeqNo sebesar +100.
    // Kita akan cek: Jika SeqNo yang ditawarkan jauh lebih besar dari yang kita tahu,
    // maka itu mencurigakan!
    
    const uint8_t advertisedHop = rrepHeader.GetHopCount();
    RoutingTableEntry toDstCheck;
    if (m_routingTable.LookupRoute (dst, toDstCheck))
    {
        const int32_t seqNoAnomalyMargin = 15;
        const uint8_t hopSuspiciousLimit = 3;

        int32_t seqDiff = static_cast<int32_t> (rrepHeader.GetDstSeqno ()) -
                          static_cast<int32_t> (toDstCheck.GetSeqNo ());

        if (seqDiff > seqNoAnomalyMargin && advertisedHop <= hopSuspiciousLimit) 
        {
            NS_LOG_WARN ("TRUST SYSTEM: ALERT! Node " << sender << " sent FAKE RREP (SeqDiff: "
                        << seqDiff << ", Hop=" << static_cast<uint32_t> (advertisedHop)
                        << "). BLOCKING!");

            UpdateTrust (sender, -0.5);
            return; 
        }
    }

    // Jika lolos pemeriksaan di atas, baru kita cek apakah node ini Trusted
    if (IsTrusted (sender))
    {
        UpdateTrust (sender, 0.25);
    }
    else
    {
        NS_LOG_LOGIC ("TRUST SYSTEM: Ignoring RREP from Untrusted Node: " << sender);
        return; // Abaikan RREP dari node yang sudah di-blacklist
    }
    // --- AKHIR MODIFIKASI TRUST ---
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(AODVTRUSTTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(AODVTRUSTTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, AODVTRUST_PORT));
        }
    }
}
...
{
    // Cek apakah tetangga ini sudah ada di tabel trust?
    if (m_trustTable.find (neighbor) == m_trustTable.end ())
      {
        // Jika belum ada, inisialisasi dengan nilai netral 0.5
        m_trustTable[neighbor] = 0.5;
      }

    // Update nilai trust (Rumus Sederhana: Trust Lama + Value Baru)
    m_trustTable[neighbor] += value;

    // Penjagaan (Clamping) agar nilai tidak lebih dari 1.0 atau kurang dari 0.0
...
bool
RoutingProtocol::IsTrusted (Ipv4Address neighbor)
{
    // Jika node tidak ada di tabel, anggap netral (Trusted)
    if (m_trustTable.find (neighbor) == m_trustTable.end ())
      {
        return true; 
      }

    // Jika nilai trust di bawah threshold (0.2), tandai sebagai JAHAT
    if (m_trustTable[neighbor] < m_trustThreshold)
      {
        NS_LOG_WARN ("TRUST SYSTEM: Node " << neighbor << " is BLACKLISTED (Low Trust)");
        return false;
      }
...
void
RoutingProtocol::BlacklistAddress (Ipv4Address neighbor)
{
    m_trustTable[neighbor] = 0.0;
    NS_LOG_WARN ("TRUST SYSTEM: Force blacklist for " << neighbor);
}
