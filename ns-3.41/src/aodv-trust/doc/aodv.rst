...
Ad Hoc On-Demand Distance Vector (AODVTRUST)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (AODVTRUST) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 AODVTRUST model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the aodv-trust-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the AODVTRUST model lives in the directory `src/aodvtrust`.

Design
++++++

Class ``ns3::aodvtrust::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::aodvtrust::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::aodvtrust::RouteInput``,
...
this functionality are attributes of ``ns3::aodvtrust::RoutingProtocol``.
...
AODVTRUST discovers routes on demand.  Therefore, the AODVTRUST model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in aodv-trust-rqueue.cc. A smart pointer to
...
* This AODVTRUST implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from aodv-trust-UU implementation and can be disabled.
...
the assertion from the AODVTRUST RFC that AODVTRUST works over UDP.  This model uses
